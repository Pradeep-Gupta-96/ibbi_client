...
const EditHighlights = () => {
    ...
    useEffect(() => {
        const apiUrl = `http://43.205.145.146:4000/highlights/gethighlightsById/${id}`;
        ...
        async function fetchTodo() {
            try {
                ...
                setTitle(data.highlights.title);
                setImage(data.highlights.image);
                setDescription(data.highlights.description);
            } catch (error) {
    ...
    const handleSubmit = async (event) => {
        try {
            ...
            const API = `http://43.205.145.146:4000/highlights/updatehighlightsById/${id}`;
